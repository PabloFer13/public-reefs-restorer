...
const mockData = [
  ...
  { img: fish, x: 100, y: 450, type: 'object', found: false, over: { type: 'sand', found: false } },
  { img: fish, x: 600, y: 450, type: 'object', found: false, over: { type: 'rock', found: false } },
  { img: fish, x: 350, y: 275, type: 'object', found: false, over: { type: 'wood', found: false } },
];

const reloadedMockData = [
  { type: 'animal', img: fish, coords: [{ x: 100, y: 100 }, { x: 600, y: 100 }, { x: 100, y: 450 }, { x: 600, y: 450 }, { x: 350, y: 275 }] },
  { type: 'night-animal', img: gFish, coords: [{ x: 100, y: 100 }, { x: 600, y: 100 }, { x: 100, y: 450 }, { x: 600, y: 450 }, { x: 350, y: 275 }] }
];
...
function HideNSeek() {
  ...
  useEffect(() => {
    const newBoard = objs.reduce((acc, it, ind) => {
      const shouldRenderAnimal = (!nightMode && it.type === 'animal') || (nightMode && it.type === 'night-animal')
    ...
    const newBlocks = objs.reduce((acc, it, ind) => {
      const { over } = it;
      const shouldRenderBlock = (!nightMode && it.type === 'animal') || (nightMode && it.type === 'night-animal')
      if(over && !it.found && !over.found && shouldRenderBlock){
        acc.push({ x: it.x, y: it.y, img: blockers[over.type], originalInd: ind, type: over.type });
      }
  ...
  const clickItem = (i) => {
    if(((objs[i].over && objs[i].over.found) || !objs[i].over) && currentTool === 'red'){
      ...
      const foundsInd = reloadedMockData.findIndex(it => it.type === objs[i].type)
  ...
  return (
    <Stage 
      ...
      width={1}
      height={1}
      filters={nightMode && !(currentTool === 'linterna') ? [lightmapFilter] : []}
    ...
    >
      ...
      {reloadedMockData.map((it, ind) => (
        <Container>
          ...
          />
          <Text position={`25,${50 + (90 * ind) }`} style={{ fontSize: 18 }} text={`${found[ind]}/${it.coords.length}`} />
        </Container>
      ))}
      <Container>
        <Text position="390,25" style={{ fontSize: 22, fill: timeLeft > 30 ? 'black' : 'white' }} text={`${timeLeft}`} />
      </Container>
      {boardObjects.map((it, ind) => {
        return (
          <Sprite
            ...
            texture={new PIXI.Texture.from(it.img)}
            interactive
            click={() => { clickItem(it.originalInd) }}
          />
        )
      })}
      {blocks.map((it, ind) => {
